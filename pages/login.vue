...
<template>
  <!-- <div class="max-w-lg mx-auto mt-8">
    ...
    <NuxtLink to="https://www.youtube.com/playlist?list=PLHIhzV3yZ5WbvbJhEbZysgmpbsLYR0Q34" target="_blank"
      class="inline-block px-4 py-2 mt-4 font-medium text-white rounded bg-green hover:bg-green-400">
      Nuxt Video Playlist
    </NuxtLink>
    <form @submit.prevent="() => (isSignUp ? signUp() : login())" class="flex flex-col gap-2 mt-16">
    ...
    <div class="w-full max-w-sm p-6 bg-white rounded-md shadow-md">
      ...
      <form class="mt-4" @submit.prevent="() => (isSignUp ? signUp() : login())">
        ...
        <div class="mt-6">
          ...
          >
          <span v-if="isSignUp"> Cadastrar-se </span>
          <span v-else> Entrar </span>
          </button>
        </div>
      </form>
      <button @click="isSignUp = !isSignUp" class="w-full mt-8 text-sm text-center underline text-slate-300">
      <span v-if="isSignUp"> Já possui uma conta? Entre </span>
      <span v-else> Criar uma nova conta </span>
    </button>
    </div>
  </div>
</template>
